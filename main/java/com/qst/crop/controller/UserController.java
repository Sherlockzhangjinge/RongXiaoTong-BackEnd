...
@RestController
@RequestMapping("/user")
@CrossOrigin//@CrossOrigin 是一个注解，用来启用跨域资源共享（CORS）功能。它允许网页浏览器从不同的域名请求资源，而不是只能从提供网页的域名请求。
// 它对于开发需要使用不同来源的API的网页应用很有用。
...
public class UserController {
 ...
 // Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
  ...
  //用户登录之后，修改用户密码
   ...
   //@RequestBody 注解则表示 passwordParam 参数是从请求体中获取的 JSON 对象，该注解告诉 Spring MVC 框架将请求体中的 JSON 数据转换成 PasswordParam 对象。
//    BindingResult 参数则用于接收校验结果，如果校验失败，会将错误信息保存在 BindingResult 对象中，开发人员可以在方法内部对错误进行处理。
    //@Validated 注解用于对 passwordParam 参数进行校验，确保其符合定义的校验规则
   public Result<String> loginUpdatePassword(@Validated @RequestBody Password password, BindingResult bindingResult) {
