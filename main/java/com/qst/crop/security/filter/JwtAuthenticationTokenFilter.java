...
//        System.out.println("token:"+token);
        ...
        //从请求头取出来Authorization；判断Authorization是不是存在
        if (!StringUtils.isEmpty(token)) {
            //从令牌中取出来 用户名
            String username = jwtTokenUtil.getUsernameFromToken(token);
            //如果用户名存在；并且SecurityContextHolder中没有存储Authentication 则表示用户没有登陆
...
//                System.out.println("userDetails"+userDetails);
                if (jwtTokenUtil.validateToken(token, userDetails)) {//验证token中的用户名和密码是否正确，与userDetails对比
                    ...
                    // 将 authentication 存入SecurityContextHolder（ ThreadLocal），方便后续获取用户信息
